...
public class JwtTokenUtil implements Serializable {
    ...
    public JwtUser getUserDetails(String token) {
        ...
        try {
            ...
            return new JwtUser(claims.getSubject(), "",
                    authorities, (boolean) claims.get(CLAIM_KEY_IS_ENABLED));
        } catch (Exception e) {
            return null;
        }
    ...
    public String generateToken(Authentication authentication) throws JsonProcessingException {
        ...
        claims.put(CLAIM_KEY_CREATED, new Date());
        List<String> auth = authentication.getAuthorities().stream().map(role-> role.getAuthority()).collect(Collectors.toList());
        claims.put(CLAIM_KEY_AUTHORITIES, auth);
        claims.put(CLAIM_KEY_IS_ENABLED, true);
